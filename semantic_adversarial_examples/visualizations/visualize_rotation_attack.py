...
def main(args):
    ...
    angle = torch.asarray([random.uniform(-0.1, 0.1), ], requires_grad=True)
    ...
    while True:
        ...
        img_t_new = kornia.geometry.warp_affine(img_t, M, dsize=(h, w))
        ...
        print(label, y_pred.item(), y_pred_new.item(), img_t.mean().item(), img_t_new.mean().item(), angle.item(), angle.grad.item())

        # factor.data = factor.data + alpha * factor.grad
        # factor.grad = None
        angle.data = torch.clamp(angle.data + args.step_size * angle.grad, min=args.alpha, max=args.beta)
        angle.grad = None
