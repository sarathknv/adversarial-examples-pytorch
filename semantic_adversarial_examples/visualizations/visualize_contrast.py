...
def main(args):
    ...
    cv2.namedWindow('img')
    cv2.createTrackbar('c (0 to 10)', window_name, 100, 1000, nothing)
    ...
    while True:
        factor = cv2.getTrackbarPos('c (0 to 10)', window_name)
        factor = (factor/100)
        print(factor)

        img_new_t = kornia.enhance.adjust_contrast(img_t, factor)
