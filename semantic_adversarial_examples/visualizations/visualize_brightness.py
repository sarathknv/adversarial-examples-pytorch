...
def main(args):
    ...
    cv2.namedWindow('img')
    cv2.createTrackbar('b (0 to 1)', window_name, 100, 200, nothing)
    ...
    while True:
        factor = cv2.getTrackbarPos('b (0 to 1)', window_name)
        factor = (factor - 100) / 100
        print(factor)

        img_new_t = kornia.enhance.adjust_brightness(img_t, factor)
