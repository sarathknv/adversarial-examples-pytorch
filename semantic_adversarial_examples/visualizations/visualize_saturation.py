...
def main(args):
    ...
    cv2.namedWindow('img')
    cv2.createTrackbar('s (0 to inf)', window_name, 100, 1000, nothing)
    ...
    while True:
        factor = cv2.getTrackbarPos('s (0 to inf)', window_name)
        factor = factor / 100  # 0 - gray; 1 - no change; 2 - multiplies saturation by 2
        print(factor)

        img_new_t = kornia.enhance.adjust_saturation(img_t, factor)
