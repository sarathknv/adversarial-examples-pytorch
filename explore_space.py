...
vectors in 32*32*4 dimensional space) we add vec2, which is inside a unit hypersphere. Radius can 
be increased by changing rad. Press 'r' and 'e' for changing vec2 and vec1 respectively.

In one of his papers, Ian Goodfellow mentions,
	'''
...
while True:
	...
	inp.data = torch.clamp(inp.data + torch.from_numpy(pert).float(), min=0, max=1)
